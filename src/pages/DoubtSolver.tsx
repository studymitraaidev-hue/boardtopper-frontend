...
const LIKELY_QUESTIONS: LikelyQuestion[] = [
  { subject: 'Mathematics', question: 'Prove that √5 is irrational using the contradiction method.', probability: '92%', icon: <Target className="w-4 h-4" /> },
...
export default function DoubtSolver() {
  ...
  useEffect(() => {
    ...
    if (saved && savedDate === today) {
      setDailyCount(parseInt(saved, 10));
    } else {
      localStorage.setItem('bt_doubt_date', today);
      localStorage.setItem('bt_doubt_count', '0');
    }
  }, []);
  ...
  const handleLikelySelect = useCallback((question: string) => {
    setQuery(question);
    textareaRef.current?.focus();
  }, []);
  ...
  const handleSend = useCallback(async () => {
    ...
    if (dailyCount >= 5) {
      alert('Daily limit reached (5/5). Upgrade to Pro for unlimited doubts.');
      return;
    }
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: generateId(), role: 'ai', text: answerText, time: formatTime() },
      ]);
      setDailyCount((prev) => {
        const next = prev + 1;
        localStorage.setItem('bt_doubt_count', next.toString());
        return next;
      });
    } catch {
      setMessages((prev) => [
        ...prev,
        {
          id: generateId(),
          role: 'ai',
          text: 'Failed to connect. Check your internet connection.',
          time: formatTime(),
        },
      ]);
    } finally {
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white shadow-lg shadow-indigo-200">
              <Sparkles className="w-4 h-4" />
            </div>
            <div>
              <h1 className="text-sm font-bold text-slate-900 leading-tight">AI Doubt Solver</h1>
              <p className="text-[10px] text-slate-500 font-medium tracking-wide uppercase">Maharashtra SSC</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => navigate('/history')} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {QUICK_PROMPTS.map((prompt, i) => (
                <SubjectCard key={prompt.label} prompt={prompt} index={i} onClick={() => handleQuickPrompt(prompt)} />
              ))}
            </div>
        ...
        <div className="max-w-2xl mx-auto px-4 py-3">
          {selectedImage && imagePreviewUrl && (
            <div className="flex items-center gap-3 px-3 py-2 mb-2 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <button onClick={clearImage} className="p-1 hover:bg-slate-200 rounded transition-colors">
          ...
          <div className="flex items-end gap-2">
            <button onClick={() => fileInputRef.current?.click()} className="flex-shrink-0 p-3 hover:bg-slate-100 rounded-xl transition-colors">
              <ImagePlus className={`w-5 h-5 ${selectedImage ? 'text-indigo-600' : 'text-slate-400'}`} />
            </button>
            <textarea
              ref={textareaRef}
              value={query}
              ...
              rows={1}
              className="flex-1 bg-transparent border-0 resize-none py-3 px-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-0 max-h-[120px]"
            />
            <button
              onClick={handleSend}
              disabled={(!query.trim() && !selectedImage) || isTyping}
              className={`flex-shrink-0 p-3 rounded-xl transition-all ${(query.trim() || selectedImage) && !isTyping ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200 hover:bg-indigo-700' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
            >
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (!f) return;
              if (!f.type.startsWith('image/')) { alert('JPG/PNG/WebP only'); return; }
              if (f.size > 5 * 1024 * 1024) { alert('Under 5MB'); return; }
              setSelectedImage(f);
